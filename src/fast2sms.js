...
function parseFast2SmsResponse(res, raw) {
    let data = {};
    try {
 ...
 */
export async function sendDltTemplateSms(input) {
    const key = apiKey();
    if (!key) {
    ...
    if (input.smsDetails)
        body.sms_details = '1';
    if (input.udf1)
        body.udf1 = input.udf1;
    if (input.udf2)
        body.udf2 = input.udf2;
    if (input.udf3)
 ...
 */
export async function sendSmsOtp(phone10, otpCode, options) {
    const key = apiKey();
    if (!key) {
        return { ok: false, error: 'Fast2SMS is not configured (FAST2SMS_API_KEY)' };
    }
    ...
    if (otpId) {
        const expiryMinutes = Math.max(1, Math.min(10080, Number(process.env.FAST2SMS_OTP_EXPIRY_MINUTES || 5)));
        try {
            const body = {
                mobile,
                otp_id: otpId,
